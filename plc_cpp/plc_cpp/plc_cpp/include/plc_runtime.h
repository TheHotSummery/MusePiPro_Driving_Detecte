...
private:
    ...
    // 同步
    // 【修复】锁顺序规则（避免死锁）：
    // 1. modbus_mutex_ (最高优先级)
    // 2. modbus_threads_mutex_
    // 3. runtime_mutex_
    // 4. yolo_heartbeat_mutex_
    // 5. gpio_mutex_ (通过 gpio_ 对象访问，最低优先级)
    // 注意：如果需要获取多个锁，必须按此顺序获取
    ...
    // Modbus 线程管理（修复资源泄漏）
    std::vector<std::thread> modbus_client_threads_;
    std::mutex modbus_threads_mutex_;
    static constexpr size_t MAX_MODBUS_THREADS = 10;  // 限制最大线程数
    void cleanup_finished_modbus_threads();  // 清理已完成的线程
    ...
    // YOLO 存活检测：记录 M39 最后写入时间
    std::chrono::steady_clock::time_point last_yolo_heartbeat_time_;
    std::mutex yolo_heartbeat_mutex_;  // 保护 last_yolo_heartbeat_time_ 的访问
};
