...
class ModelManager:
    ...
    def postprocess_onnx(self, outputs, conf_threshold, iou_threshold, num_classes, img_h, img_w):
        ...
        boxes_raw_all = np.clip(boxes_raw_all, 0, MODEL_INPUT_SIZE[1] - 1)
        ...
        boost_mask = (focused_conf > conf_threshold - CONFIG["focused_confidence_boost"]) & (conf_diff < 0.1)
        ...
        candidate_indices = np.where(final_confidences_all > conf_threshold)[0]
        ...
        x1 = np.clip(x1, 0, MODEL_INPUT_SIZE[1] - 1)
        y1 = np.clip(y1, 0, MODEL_INPUT_SIZE[0] - 1)
        x2 = np.clip(x2, 0, MODEL_INPUT_SIZE[1] - 1)
        y2 = np.clip(y2, 0, MODEL_INPUT_SIZE[0] - 1)
