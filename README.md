...
### V2.3.2 重写了后端数据接收和数据展示，新增了一键启动
- **一键启动**：上电自动加载plc和yolo程序，无需繁琐配置，开机即用。
#### 启动状态指示
- **系统上电开机**：指示灯常亮
- **PLC 就绪（开机约 2 分钟）**：指示灯快闪
- **YOLO 就绪（再约 1 分钟）**：指示灯慢闪，系统正常工作
#### 设计优化
- **通体铝合金**：抗电磁干扰，防尘，顺带提供散热（无需机械风扇，温度不超过60）。
- **拔插便捷**：全部采用拔插型航空插头，连接稳定，输出电流大。
#### 展示优化
- **炫酷ui**：基于Tailwind css实现车辆轨迹展现，疲劳点位显示，时间段热力图展示等。
