...
本项目是一款基于国产嘉立创，专为微控制器（尤其是Muse Pi Pro，等io驱动能力比较弱的）、PLC、树莓派、ESP32等主控板设计的**多通道光耦隔离数字量 I/O 扩展模块**。它提供了3路隔离输入和6路大功率隔离输出，旨在安全地连接低压控制系统与较高电压的工业现场设备，有效消除地线回路干扰，防止高压浪涌对主控器的损害，极大增强系统的可靠性和安全性。

板载高质量的电源转换电路、全面的保护措施和灵活的逻辑极性配置，使其成为自动化控制、智能家居、设备改造等领域的理想选择。
...
*   **大功率输出**：输出通道采用高性能的 **N-Channel MOSFET (TPH1R403NL)**，能够驱动高达12V的直流负载，如继电器、电磁阀、直流电机、LED灯带等。
*   **灵活的逻辑配置**：
...
*   **专业PCB设计**：布局清晰，丝印详尽，高低压区域明确分离，确保性能和安全。
...
这是本模块最灵活的部分，请根据您的需求进行配置。丝印上的“**高**”代表高电平有效，“**低**”代表低电平有效。
...
| 跳线位置 | 功能 | 描述 |
| :--- | :--- | :--- |
| **连接 1-2** | **低电平有效 (LOW)** | 当 `EN` 引脚为低电平时，允许所有输出通道工作。 |
...
| **连接 2-3** | **低电平有效 (LOW)** | 当对应的控制引脚为低电平时，该路MOSFET导通，负载工作。 |
